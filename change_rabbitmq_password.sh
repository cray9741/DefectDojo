...
PASSWORD=$(kubectl get secret defectdojo-rabbitmq-specific -o jsonpath="{.data.rabbitmq-password}" | base64 --decode)

# Step 2: Execute the command in the defectdojo-rabbitmq-0 pod
kubectl exec -it defectdojo-rabbitmq-0 -n default -- /bin/bash -c "
