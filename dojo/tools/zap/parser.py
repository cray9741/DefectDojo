...
class ZapParser:
    ...
    MAPPING_SEVERITY = {"0": "Low", "1": "Medium", "2": "High", "3": "Critical"}
    MAPPING_CONFIDENCE = {
    ...
    def get_findings(self, file, test):
        ...
        for node in tree.findall("site"):
            for item in node.findall("alerts/alertitem"):
                finding = Finding(
                    test=test,
                    title=html.unescape(item.findtext("alert")),
                    description=html2text(html.unescape(item.findtext("desc"))),
                    ...
                    mitigation=html2text(html.unescape(item.findtext("solution"))),
                    references=html2text(html.unescape(item.findtext("reference"))),
                ...
                for instance in item.findall("instances/instance"):
                    endpoint = Endpoint.from_uri(html.unescape(instance.findtext("uri")))
                    ...
                    if instance.findtext("requestheader") is not None:
                        ...
                        request = html.unescape(instance.findtext("requestheader")) + html.unescape(instance.findtext("requestbody"))
                        response = html.unescape(instance.findtext("responseheader")) + html.unescape(instance.findtext("responsebody"))
                    else:
                        ...
                        # and response fields.
                        request = f"Method:           {html.unescape(instance.findtext('method'))} \nParam:            {html.unescape(instance.findtext('param'))} \nAttack:           {html.unescape(instance.findtext('attack'))} \nEndpointQuery:    {endpoint.query} \nEndpointFragment: {endpoint.fragment}"
                        response = f"{html.unescape(instance.findtext('evidence'))}"
